...
- **`api loop`** — el bucle gobernado: Sequentia recupera y verifica, **tu** modelo genera, y la decisión sale de una sola cifra de política (`--max-send-risk`, por defecto 0.5) **cuando la KB respalda la respuesta**. Dos veredictos no la miran: `contradicted` escala siempre —no es incertidumbre, es un desacuerdo entre la KB y lo que el modelo escribió— y `unsupported` también, porque que la KB no sostenga algo no se compensa con un riesgo bajo. El umbral decide entre mandar y escalar sólo dentro de `supported`. Sirve cualquier `/chat/completions` compatible con OpenAI — vLLM, Ollama, LM Studio, OpenRouter u OpenAI directo. **Azure no entra en este shape**: necesita endpoint, apiVersion y deployment.
...
- **`api collection --check`** — caza la deriva entre la copia empaquetada y la que sirve tu celda comparando el **catálogo**, no el JSON crudo: un export de Postman agrega `_postman_id`, `uid`, `owner` y marcas de tiempo, así que un diff textual estaría siempre en rojo, que es la forma más común de que un control deje de controlar.
...
- **La colección llegó a estar publicada a mano en un workspace público de Postman**, y de ahí salieron las tres trampas que `collection/PUBLISHING.md` conserva: un import puede crear una colección **paralela** en vez de reemplazar, la interfaz puede ir minutos por detrás de la nube, y por las dos anteriores la pantalla no sirve para dar por buena una publicación. Ya no es parte del ciclo: quien quiera el workspace lo importa desde la URL de su celda.
